...
export const uploadImage = (id, image) => async dispatch => {
  try {
    ...
    if (response.ok) {
      dispatch(setAlert('Image chargé', 'success', 5000));
    }
...
export const login = (username, password) => async dispatch => {
  try {
    ...
    if (response.ok) {
      ...
      dispatch(setAlert(`Bienvenue ${user.name}`, 'success', 5000));
    }
    if (responseData.error) {
...
export const register = ({
  ...
  try {
    ...
    if (response.ok) {
      ...
      dispatch(setAlert('Inscription réussie', 'success', 5000));
    }
...
export const logout = () => async dispatch => {
  try {
    ...
    if (response.ok) {
      ...
      dispatch(setAlert('Déconnexion réussie', 'success', 5000));
    }
    if (responseData.error) {
      dispatch(setAlert(responseData.error.message, 'error', 5000));
    }
